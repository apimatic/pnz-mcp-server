...
const getPartyInputSchema = await schemas.loadSchema("Party", "GetParty");

interface GetPartyMCPToolRequest {
...
async function executeGetParty(
  req: GetPartyMCPToolRequest
): Promise<CallToolResult> {
  try {
    const { result } = await partyController.getParty(
...
export async function handleGetParty(
  args: unknown | GetPartyMCPToolRequest
): Promise<CallToolResult> {
  const validationSummary = schemas.validate(args, "Party", "GetParty");
  if (!validationSummary.isValid) {
    ...
    return {
      content: [{ type: "text", text: "Invalid arguments for getParty tool." }],
      isError: true,
    };
  }

  return await executeGetParty(args as GetPartyMCPToolRequest);
}
...
export const getPartyTool: ToolDefinition = {
  tool: {
    ...
    inputSchema: getPartyInputSchema,
  },
