...
const getBalancesInputSchema = await schemas.loadSchema("Balances", "GetBalances");
...
async function executeGetBalances(
  ...
  try {
    const { result } = await balancesController.getBalances(
...
export async function handleGetBalances(
  ...
  const validationSummary = schemas.validate(args, "Balances", "GetBalances");
