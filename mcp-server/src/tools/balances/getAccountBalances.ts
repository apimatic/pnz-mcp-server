...
const getAccountBalancesInputSchema = await schemas.loadSchema("Balances", "GetAccountBalances");

interface GetAccountBalancesMCPToolRequest {
  'AccountId': string,
...
async function executeGetAccountBalances(
  req: GetAccountBalancesMCPToolRequest
): Promise<CallToolResult> {
  try {
    const { result } = await balancesController.getAccountBalances(
      req['AccountId'],
...
export async function handleGetAccountBalances(
  args: unknown | GetAccountBalancesMCPToolRequest
): Promise<CallToolResult> {
  const validationSummary = schemas.validate(args, "Balances", "GetAccountBalances");
  if (!validationSummary.isValid) {
    ...
    return {
      content: [{ type: "text", text: "Invalid arguments for getAccountBalances tool." }],
      isError: true,
    };
  }

  return await executeGetAccountBalances(args as GetAccountBalancesMCPToolRequest);
}

export const getAccountBalancesTool: ToolDefinition = {
  tool: {
    name: "balances-getAccountBalances",
    description: "Get Balances related to an account",
    inputSchema: getAccountBalancesInputSchema,
  },
  handler: handleGetAccountBalances,
};
