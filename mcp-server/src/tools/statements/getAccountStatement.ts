...
const getAccountStatementInputSchema = await schemas.loadSchema("Statements", "GetAccountStatement");

interface GetAccountStatementMCPToolRequest {
  'AccountId': string,
  'StatementId': string,
...
async function executeGetAccountStatement(
  req: GetAccountStatementMCPToolRequest
): Promise<CallToolResult> {
  try {
    const { result } = await statementsController.getAccountStatement(
      req['AccountId'],
      req['StatementId'],
...
export async function handleGetAccountStatement(
  args: unknown | GetAccountStatementMCPToolRequest
): Promise<CallToolResult> {
  const validationSummary = schemas.validate(args, "Statements", "GetAccountStatement");
  if (!validationSummary.isValid) {
    ...
    return {
      content: [{ type: "text", text: "Invalid arguments for getAccountStatement tool." }],
      isError: true,
    };
  }

  return await executeGetAccountStatement(args as GetAccountStatementMCPToolRequest);
}

export const getAccountStatementTool: ToolDefinition = {
  tool: {
    name: "statements-getAccountStatement",
    description: "Get Statement related to an account",
    inputSchema: getAccountStatementInputSchema,
  },
  handler: handleGetAccountStatement,
};
